...
# ela vem do nasa power
env.data = get_weather(env.id = 'NAIROBI',
                       country = 'KEN',
                       lat = -1.367,lon = 36.834,
                       start.day = '2015-03-01',
                       end.day = '2015-04-01')

# data = YYYY-MM-DD

# Longitude Leste = positivo
# Longitude Oeste = negativo
# Latitude Norte  = positivo
...
clay_5_15 <- raster('out')

clay_data = extract_GIS(covraster = clay_5_15,name.out = 'clay_5_15',env.data = env.data)
head(clay_data)


meus_rasters <- list.files(path = '/suapasta')
nomes_dos_rasters <-c('argila_5cm',"nitrogenio_5cm",'densidade_5cm')

for(i in 1:length(meus_raster))
{
  raster_file <- raster(meus_rasters[i])
  clay_data = extract_GIS(covraster = raster_file ,name.out =nomes_dos_rasters[i] ,env.data = env.data)
...
env = c('GOI','TEX','BRI','MON','LOS','PON','CAL','PAL','VIC_1','VIC_2')

lat = c(-16.67,19.25,-27.47,43.61,14.170,6.294,3.261,-10.168,-20.7546,-20.7546)

lon = c(-49.25,-99.50,153.02,3.87,121.241,2.361,-76.312,-48.331,-42.8825,-42.8825)

cou = c('BRA','MEX','AUS','FRA','PHL','BEN','COL','BRA','BRA','BRA')

start = c('2020-03-15','2019-05-15','2018-09-15',
          '2017-06-18','2017-05-18','2016-07-18',
          '2017-11-18','2017-12-18','2017-12-18','2018-12-18')

end = c('2020-04-15','2019-06-15','2018-10-15',
        '2017-07-18','2017-06-18','2016-08-18',
        '2017-12-18','2018-01-18','2018-01-18','2019-01-19')
...
TempData = param_temperature(env.data = env.data,Tmax = 'T2M_MAX',Tbase1 = 8,Tbase2 = 45,Topt1 = 30,Topt2 = 35)
head(TempData)
...
env.data_2 = get_weather(env.id = 'LOSBANOS',#country = 'PHL',
...
env_typing(env.data = env.data_2,env.id = 'env', var.id = 'T2M', cardinals = card)
...
env_typing(env.data = env.data_2,env.id = 'env', var.id = var, cardinals = card)


card = list(PRECTOT = NULL, T2MDEW = NULL) # cardinals and data-driven limits
env_typing(env.data = env.data_2,env.id = 'env', var.id = var, cardinals = card)
...
varid= names(env.data)[c(9,11,15)]
ET = env_typing(env.data = env.data,env.id = 'env',var.id = varid,format = 'wide')
...
EC = W_matrix(env.data = env.data,var.id = varid)
...
#'============================================================================
#### "Brincadeira" N2 = efeito da sazonalidade entre anos
#'============================================================================



(env = paste0('Vicosa_',1990:2019))

(lat = rep(-20.7546,length(1990:2019)))

(lon = rep(-42.8825,length(1990:2019)))

(cou = rep('BRA',length(1990:2019)))

(start = paste0(1990:2019,'-12-18')) # 18 de dezembro

(end =  paste0(1991:2020,'-03-15'))  #  15 de marco
...
# QUAL EH A SUA CONCLUSAO?
# E SE COLOCASSEMOS MAIS VARIAVEIS AMBIENTAIS?
